...
<head>
    <title>
        {{ \App\Models\Utility::getValByName('title_text') ? \App\Models\Utility::getValByName('title_text') : config('app.name', 'HR Tool') }}
    ...
    <meta property="og:image"
        content="{{ isset($meta_logo) && !empty(asset('storage/uploads/meta/' . $meta_logo)) ? asset('storage/uploads/meta/' . $meta_logo) : 'hr.png' }}">
    ...
    <div class="custom-login">
        <div class="login-bg-img">
            ...
            @if (strpos($themeColor, 'theme') === 0)
                <img src="{{ asset('assets/images/' . $themeColor . '.png') }}" class="login-bg-1">
            @else
                <img src="{{ asset('assets/images/.svg') }}" class="login-bg-1">
            @endif

            <img src="{{ asset('assets/images/hr-01.jpg') }}" class="login-bg-2">
        </div>
                        ...
                        <div class="navbar-brand">
                            <a href="#">
                                <img src="{{ $logo . '/' . (isset($company_logo) && !empty($company_logo) ? $company_logo . '?' . time() : 'logo_dark.png' . '?' . time()) }}"
                                    class="logo" alt="{{ config('app.name', 'HR Tool') }}" alt="logo"
                            ...
                            <div class="col-12">
                                <span>
                                    @if (empty(App\Models\Utility::getValByName('footer_text')))
                                        &copy;{{ date(' Y') }}
                                    @endif
                                    {{ App\Models\Utility::getValByName('footer_text') ? App\Models\Utility::getValByName('footer_text') : config('app.name', 'HR Tool') }}
                                </span>
                            </div>
                        </div>
                    </div>
